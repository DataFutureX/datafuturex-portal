...
export const works: Work[] = [
  {
    ...
    quickStart: [
      ...
      'cd yunqi-application-platform/frontend && npm install && npm run dev:demo',
      '打开 http://localhost:3000 ，账号 demo / demo123',
    ],
    links: {
      demo: 'https://yunqi.datafuturex.cn/portal',
      github: 'https://github.com/DataFutureX/yunqi-application-platform',
      gitee: 'https://gitee.com/DataFutureX/yunqi-application-platform',
      docsAnchor: '/docs/yunqi',
    },
    screenshots: [
      { src: '/works/yunqi-application-platform/02-login.webp', alt: '登录页' },
      { src: '/works/yunqi-application-platform/03-dashboard.webp', alt: '工作台仪表盘' },
      { src: '/works/yunqi-application-platform/04-user.webp', alt: '用户管理' },
      { src: '/works/yunqi-application-platform/05-unit.webp', alt: '单位管理' },
      { src: '/works/yunqi-application-platform/06-role.webp', alt: '角色授权' },
      { src: '/works/yunqi-application-platform/07-menu.webp', alt: '菜单配置' },
      { src: '/works/yunqi-application-platform/08-system-config.webp', alt: '系统设置' },
      { src: '/works/yunqi-application-platform/09-announcement.webp', alt: '公告管理' },
      { src: '/works/yunqi-application-platform/10-operation-log.webp', alt: '操作日志' },
      { src: '/works/yunqi-application-platform/11-monitor.webp', alt: '系统监控' },
      { src: '/works/yunqi-application-platform/12-api-docs.webp', alt: '接口文档' },
      { src: '/works/yunqi-application-platform/13-profile.webp', alt: '个人信息' },
      { src: '/works/yunqi-application-platform/14-change-password.webp', alt: '修改密码' },
    ],
    logo: '/works/yunqi-application-platform/logo.svg',
    accounts: [
  ...
  },
  {
    ...
    tag: 'v1.0.0 · 暂未开源',
    summary:
      '从设备接入到行业应用的 AI 物联控制面：设备接入 → 数据采集/治理 → 规则决策 → 行业应用；控制台四大业务域为设备管理 · 数据中心 · 规则引擎 · 平台设置（另含主页工作台 / 应用市场）。',
    openSource: false,
    accessNote:
      '暂未开源。在线演示为纯前端 Mock：https://iot.datafuturex.cn/portal 。基于开源底座云起应用平台（yunqi-application-platform / YQAP）演进。AI 分析与数据孪生为规划能力，未写入当前交付范围。',
    ...
    capabilities: [
      '设备管理：协议 · 产品 · 物模型 · Topic · 设备 · 影子 · 指令 · 网关/子设备 · CSV 批量 · OTA；MQTT 为主接入（HTTP 可选默认关；Modbus 为 stub；TCP/CUSTOM 占位）',
      '数据中心：历史 / 事件 / 实时 / 时序运维与保留策略；默认 PostgreSQL，可切 TDengine',
      '规则引擎：属性/事件/在离线规则，告警中心，通道转发（JDBC/Redis/Kafka/HTTP/MQTT）',
      '主页：工作台健康概览；应用市场孵化行业应用并支持启停',
      '平台设置：多租户 RBAC（租户编码登录）、应用凭证、公告 SSE、操作日志、系统监控、内嵌 Swagger',
      '产品门户 /portal 与设备模拟器 iot-simulator（DEV001–DEV020）已落地',
    ],
    scenarios: [
      '工业现场按「协议 → 产品（物模型 + Topic）→ 设备」纳管，MQTT 联调心跳/属性/影子/指令',
      '时序与事件统一查询，规则告警与多通道数据转发',
